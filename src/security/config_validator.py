...
from src.validators.input_validator import validate_cors_origin, validate_api_key_format
...
class SecurityConfigValidator:
    ...
    def _validate_api_key_settings(self, settings: Settings):
        ...
        if not settings.api_key:
            ...
            if settings.api_key.lower() in ['test', 'demo', 'example', 'your_api_key_here']:
                self.issues.append({
                    "category": "Authentication",
                    "severity": "CRITICAL",
    ...
    def _validate_openai_settings(self, settings: Settings):
        ...
        elif settings.openai_api_key.startswith('your_') or 'example' in settings.openai_api_key.lower():
            self.issues.append({
                "category": "OpenAI",
                "severity": "CRITICAL",
    ...
    def _calculate_security_score(self) -> int:
        ...
        normal_issues = [i for i in self.issues if i.get('severity') not in ['CRITICAL', 'HIGH']]
