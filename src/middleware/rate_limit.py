...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if self._is_rate_limited(client_id):
            ...
            retry_after = int(
                oldest_request + self.window_size - time.time()
            )
        ...
        if (len(self.request_times) > 0 and 
            len(self.request_times) % 100 == 0):
        ...
        remaining = max(
            0, 
            self.calls_per_minute - len(self.request_times[client_id])
        )
