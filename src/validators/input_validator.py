...
from src.models.message import Message, MessageRole, ChatRequest
...
class SecurityValidator:
    ...
    def validate_messages_sequence(self, messages: List[Message]) -> tuple[bool, Optional[str]]:
        """
        ...
        if len(messages) > self.max_messages_count:
            return False, f"Слишком много сообщений (максимум {self.max_messages_count})"
        ...
        if consecutive_user_count > self.max_consecutive_user_messages:
            return False, f"Слишком много последовательных сообщений пользователя (максимум {self.max_consecutive_user_messages})"
...
class EnhancedChatRequest(ChatRequest):
    ...
    def validate_messages_security(cls, v):
        ...
        for message in v:
            is_valid, error = security_validator.validate_message_content(message.content)
            if not is_valid:
...
def validate_request_data(data: Dict[str, Any]) -> tuple[bool, Optional[str], Optional[EnhancedChatRequest]]:
    """
