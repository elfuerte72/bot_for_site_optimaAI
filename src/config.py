...
class Settings(BaseSettings):
    ...
    openai_api_key: str = Field(
        ..., min_length=10, description="OpenAI API ключ"
    )
    gpt_model: str = Field(
        default=DEFAULT_GPT_MODEL, description="Модель GPT"
    )
...
Всегда отвечай на русском языке.""",
    ...
    app_name: str = Field(
        default="OptimaAI Bot", description="Название приложения"
    )
    debug: bool = Field(default=False, description="Режим отладки")
    host: str = Field(default="0.0.0.0", description="Хост для запуска")
    port: int = Field(
        default=8000, ge=1, le=65535, description="Порт для запуска"
    )
    ...
    cache_ttl_seconds: int = Field(
        ...
        ge=0, 
        description="TTL кэша в секундах"
    )
    ...
    rag_chunk_size: int = Field(
        ...
        ge=100, 
        le=2000, 
        description="Размер чанка для RAG"
    )
    rag_chunk_overlap: int = Field(
        default=200, 
        ge=0, 
        le=500, 
        description="Перекрытие чанков для RAG"
    )
    ...
    persist_dir: str = Field(
        default="rag_index", description="Директория для индексов"
    )
    ...
    def validate_openai_key(cls, v):
        ...
        if not v or len(v) < 10:
            raise ValueError(
                'OpenAI API ключ должен быть указан и содержать '
                'минимум 10 символов'
            )
        return v
    ...
    def validate_origins(cls, v):
        ...
        if not v:
            raise ValueError(
                'Должен быть указан хотя бы один разрешённый домен'
            )
        return v
